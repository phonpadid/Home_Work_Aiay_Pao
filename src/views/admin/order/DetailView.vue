...
onMounted(() => {
  if (route.params.id) {
    const orderIdParam = Array.isArray(route.params.id)
      ? route.params.id[0]
      : route.params.id;

    orderId.value = parseInt(orderIdParam, 10);
    ...
    <div class="flex pt-8 px-6">
      <div class="card w-full p-6 bg-base-100 shadow-xl">
        ...
        <div v-for="(product, index) in orderData.products" class="flex items-center my-2" :key="index">
          ...
          <div class="flex-1">ຈຳນວນ {{ product.quantity }} ອັນ</div>
          <div>{{ product.price }} ฿</div>
        </div>
        <div class="divider"></div>
        <div class="flex align-middle justify-between mb-2">
          <div class="font-bold">ລາຄາສິນຄ້າທັງໝົດ</div>
